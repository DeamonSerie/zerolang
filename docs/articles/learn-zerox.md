...
Output is not magic in Zerox. The program writes through `world.out`:
...
`let byte u8 256` fails at `zerox check`.
...
Zerox has ordinary `if` / `else` blocks:
...
pub fn main Void world World !
  ...
  if first.has
    let written std.fs.write ".zerox/out/name.txt" first.value
    if > written 0
...
A package has a `zerox.json` manifest and source files under `src/`.
...
```sh
zerox check examples/systems-package
```

## Run Tests

Zerox test blocks live next to source code:
...
```sh
zerox test conformance/native/pass/test-blocks.0
zerox test --json --filter addition conformance/native/pass/test-blocks.0
```
...
Target names are explicit. Use `zerox targets` to inspect support, then pass `--target` to `check`, `build`, `graph`, or `size`:

```sh
zerox targets
zerox check --target linux-musl-x64 examples/memory-package
zerox build --target linux-musl-x64 examples/memory-package --out .zerox/out/memory-package
```
...
zerox explain NAM003
zerox fix --plan --json conformance/check/fail/unknown-name.0
```
...
Some examples introduce the vocabulary used by lower-level Zerox code:
